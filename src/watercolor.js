...
export function paintPolygon(ctx, basePoints, opts = {}) {
  const {
    ...
    layerOpacity = 0.036,
    bleed = 0.05,
    boundaryMode = 'radial', // 'radial' (blobs/bars) | 'outline' (area paths)
    wobble = 0.08,
    blooms = 5,
    bloomStrength = 1.15,
    edgeDarkening = 1.0,
  ...
  let boundary;
  if (boundaryMode === 'outline') {
    // For arbitrary outlines (area paths, ridgelines): wobble each vertex along
    // the local NORMAL by a smooth noise field, so the given SHAPE is preserved
    // (flat baselines stay flat, the data profile stays faithful) but the edge
    // gets a gentle hand-painted waver. Light smoothing only — heavy rounding
    // would erase the data.
    const n = basePoints.length;
    const disp = basePoints.map(([x, y], i) => {
      const p = basePoints[(i - 1 + n) % n];
      const q = basePoints[(i + 1) % n];
      let nx = -(q[1] - p[1]);
      let ny = q[0] - p[0];
      const L = Math.hypot(nx, ny) || 1;
      nx /= L;
      ny /= L;
      const w = (fbm(x * 0.025 + 3, y * 0.025 + 3, seed + 3, 2) - 0.5) * 2 * r * bleed;
      return [x + nx * w, y + ny * w];
    });
    boundary = chaikin(disp, 1);
  } else {
    // Blobs/bars: perturb each vertex's RADIUS with a smooth noise field sampled
    // around the ring → gentle, low-frequency hand-painted undulation (a few
    // soft lobes), NOT the regular high-frequency scallops midpoint
